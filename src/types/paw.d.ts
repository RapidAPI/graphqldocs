...
export type MapKeyedWithString<T> = Record<string, T>
export type GroupedRequestType = { group: string; path: string }
export type CreateRequestGroupType = {
  group: string
  paths: string[]
}

export type TransformStringType = {
  defaultValue: string
...
export class DynamicValue {
  public type: string
  constructor(type: string, properties?: Record<string, unknown>)
  getEvaluatedString(): string
  copy(): DynamicValue
  variableUUID: string
...
export class NetworkHTTPRequest {}
...
export declare namespace Paw {
  ...
  class RequestTreeItem {
    ...
    getUrlParameters(isDynamic?: boolean): {
  ...
  class Request extends RequestTreeItem {
    ...
    getMultipartBody(
      isDynamic?: boolean,
    ): Record<string, string | DynamicString> | null
  ...
  class Environment {
    ...
    order: number
    getVariablesValues(isDynamic?: boolean): {
      [key: string]: string | DynamicString
    }
